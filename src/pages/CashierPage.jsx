...
import { filterDni } from '../lib/inputFilters'
...
function resolveCashierEvent(session) {
  ...
  return {
    ...
    club: String(suppliedEvent.club || suppliedEvent.nombre || session?.nombre || 'CAJA'),
...
function CashierFeedback({ feedback, canUndo, onClose, onUndo }) {
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-white p-6 text-center dark:bg-void" role="alert" aria-live="assertive"><div className="w-full max-w-sm"><div className={`mx-auto grid size-36 place-items-center border-4 ${border} ${color} shadow-[10px_10px_0_rgba(139,92,246,.28)]`}><Icon name={isError ? 'close' : isUndone ? 'back' : 'check'} size={72} strokeWidth={2.5}/></div><p className={`display-title mt-10 text-5xl ${color}`}>{isError ? 'ERROR' : isUndone ? 'DESHECHO' : 'OPERACIÓN OK'}</p><p className="mt-4 font-mono text-sm font-bold uppercase leading-6 text-gray-900 dark:text-paper-text">{feedback.message}</p>{feedback.detail && <p className="mt-3 text-sm leading-6 text-gray-500 dark:text-muted">{feedback.detail}</p>}{canUndo && <button type="button" onClick={() => onUndo(feedback.transaction)} className="mt-8 min-h-14 w-full border border-amber-500 font-mono text-xs font-bold uppercase text-amber-600 dark:border-amber-300 dark:text-amber-300">Deshacer operación</button>}<button type="button" onClick={onClose} className={`mt-3 min-h-16 w-full border-2 ${border} font-mono text-sm font-bold uppercase ${color}`}>Continuar</button></div></div>
}
...
export default function CashierPage() {
  ...
  const isWeb = selected?.tipo_ingreso === 'web_anticipada'
  const isList = selected?.tipo_ingreso === 'lista_rrpp'
  const isVentaGeneral = selected?.tipo_ingreso === 'venta_general'
  ...
  return <main className="min-h-[100dvh] bg-gray-100 sm:py-5 dark:bg-black">{!eventId ? <div className="grid min-h-[100dvh] place-items-center text-center"><div><Icon name="cash" size={40} className="mx-auto text-gray-400 dark:text-muted"/><p className="mt-5 font-display text-2xl text-gray-900 dark:text-paper-text">SIN EVENTO ASIGNADO</p><p className="mt-3 max-w-xs text-sm leading-6 text-gray-500 dark:text-muted">No tenés un evento activo asignado. Pedile al organizador que te asigne uno y volvé a iniciar sesión.</p><button type="button" onClick={logout} className="btn-primary mt-7">CERRAR SESIÓN</button></div></div> : <div className="relative mx-auto min-h-[100dvh] w-full max-w-[620px] overflow-hidden bg-white sm:min-h-[calc(100dvh-40px)] sm:border sm:border-gray-200 dark:bg-void dark:sm:border-white/10"><header className="sticky top-0 z-40 border-b border-gray-200 bg-white/95 px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))] backdrop-blur dark:border-white/10 dark:bg-void/95"><div className="flex items-center gap-3"><div className="grid size-10 shrink-0 place-items-center border border-strobe text-strobe"><Icon name="cash" size={21}/></div><div className="min-w-0"><p className="truncate font-display text-lg leading-none text-gray-900 dark:text-paper-text">CAJA · {cashierEvent.club}</p><p className="mt-1 truncate font-mono text-[9px] uppercase tracking-[.16em] text-gray-500 dark:text-muted">{session?.nombre} · Terminal de cobro</p></div><button type="button" onClick={toggleTheme} className="ml-auto grid size-10 shrink-0 place-items-center border border-gray-200 text-gray-500 hover:border-strobe hover:text-strobe dark:border-white/15 dark:text-muted" aria-label={isDark ? 'Modo claro' : 'Modo oscuro'}>{isDark ? '☀️' : '🌙'}</button><button type="button" onClick={logout} className="grid size-10 shrink-0 place-items-center border border-gray-200 text-gray-500 hover:border-door-red hover:text-door-red dark:border-white/15 dark:text-muted" aria-label="Cerrar sesión"><Icon name="logout" size={18}/></button></div></header>

  <section className="border-b border-gray-200 bg-gray-50 px-4 py-4 dark:border-white/10 dark:bg-floor"><div className="flex items-end justify-between"><div><div className="flex items-center gap-2"><span className={capacityStatus === 'live' ? 'status-dot' : 'size-2 bg-gray-400 dark:bg-muted'}/><p className="font-mono text-[9px] font-bold uppercase tracking-[.16em] text-gray-500 dark:text-muted">{capacityLabel}</p></div><p className="mt-2 font-display text-4xl text-strobe">{capacity ? capacity.ingresados : '—'}<span className="ml-2 text-xl text-gray-400 dark:text-muted">/ {capacity ? capacity.aforo_max : '—'}</span></p></div><div className="text-right"><p className="font-display text-3xl text-gray-900 dark:text-paper-text">{occupancy == null ? '—' : `${occupancy}%`}</p><p className="font-mono text-[9px] uppercase text-gray-500 dark:text-muted">ocupación</p></div></div><div className="mt-3 h-2 bg-gray-200 dark:bg-void"><div className="h-full bg-gradient-to-r from-uv to-strobe transition-[width] duration-500" style={{ width: `${occupancy || 0}%` }}/></div></section>

  <div className="p-4 pb-[max(28px,env(safe-area-inset-bottom))]"><section aria-labelledby="scanner-title"><div className="mb-3 flex items-end justify-between"><div><p className="eyebrow">Identificar</p><h1 id="scanner-title" className="display-title mt-2 text-3xl text-gray-900 dark:text-paper-text">ESCANEAR ENTRADA</h1></div><span className="font-mono text-[9px] uppercase text-emerald-600 dark:text-emerald-300">Cámara activa</span></div><GuardQrScanner key={scannerKey} active={!busy && !feedback && !saleOpen} onScan={handleQrScan}/></section>

  <form onSubmit={searchDni} className="mt-4 border border-gray-200 bg-gray-50 p-3 dark:border-white/15 dark:bg-floor"><label className="font-mono text-[9px] font-bold uppercase tracking-wider text-gray-500 dark:text-muted">Buscar lista por DNI</label><div className="mt-2 flex gap-2"><input inputMode="numeric" maxLength={8} value={dni} onChange={(event) => { setDni(filterDni(event.target.value)); setInlineError('') }} className="field min-h-14 flex-1 text-center text-lg tracking-wider" placeholder="38 987 654"/><button disabled={busy} className="grid min-h-14 w-16 place-items-center bg-gray-900 text-white disabled:opacity-40 dark:bg-white dark:text-void" aria-label="Buscar DNI"><Icon name="search" size={22}/></button></div></form>
  ...
  <section className="mt-4 min-h-40 border-2 border-gray-200 bg-gray-50 p-5 dark:border-white/15 dark:bg-floor" aria-live="polite">{selected ? <><div className="flex items-center justify-between"><p className="eyebrow">Resultado inmediato</p><span className={`border px-2 py-1 font-mono text-[9px] font-bold uppercase ${isWeb ? 'border-strobe text-strobe' : isVentaGeneral ? 'border-emerald-500 text-emerald-600 dark:border-emerald-400 dark:text-emerald-300' : 'border-amber-500 text-amber-600 dark:border-amber-300 dark:text-amber-300'}`}>{isWeb ? 'Entrada web' : isVentaGeneral ? 'Venta Gral' : 'Lista RRPP'}</span></div><h2 className="display-title mt-4 text-4xl text-gray-900 dark:text-paper-text">{selected.nombre} {selected.apellido}</h2><div className="mt-5 grid grid-cols-2 gap-4 font-mono"><div><p className="text-[9px] uppercase text-gray-500 dark:text-muted">DNI</p><p className="mt-1 text-sm font-bold text-gray-900 dark:text-paper-text">{selected.dni}</p></div><div><p className="text-[9px] uppercase text-gray-500 dark:text-muted">Estado</p><p className="mt-1 text-sm font-bold text-gray-900 dark:text-paper-text">{String(selected.estado).replaceAll('_', ' ')}</p></div>{isList && <><div><p className="text-[9px] uppercase text-gray-500 dark:text-muted">RRPP</p><p className="mt-1 text-sm font-bold text-gray-900 dark:text-paper-text">{selected.rrpp_nombre}</p></div><div><p className="text-[9px] uppercase text-gray-500 dark:text-muted">A cobrar</p><p className="mt-1 text-sm font-bold text-strobe">{formatMoney(selected.monto_pago)}</p></div></>}</div></> : <div className="grid min-h-32 place-items-center text-center"><div><Icon name="ticket" size={30} className="mx-auto text-gray-400 dark:text-muted"/><p className="mt-3 font-mono text-[10px] uppercase leading-5 text-gray-500 dark:text-muted">Escaneá un QR o buscá un DNI.<br/>El resultado aparece acá.</p></div></div>}</section>
  ...
  {lastTransaction && undoRemaining > 0 && <section className="mt-4 border border-amber-500/60 bg-amber-500/5 p-4 dark:border-amber-300/60 dark:bg-amber-300/5"><div className="flex items-center gap-3"><Icon name="clock" className="shrink-0 text-amber-600 dark:text-amber-300"/><div className="min-w-0 flex-1"><p className="font-mono text-[9px] font-bold uppercase text-amber-600 dark:text-amber-300">Última operación · {formatCountdown(undoRemaining)}</p><p className="mt-1 truncate text-xs text-gray-500 dark:text-muted">{lastTransaction.detail}</p></div><button type="button" disabled={busy} onClick={() => undoTransaction()} className="min-h-11 border border-amber-500 px-3 font-mono text-[9px] font-bold uppercase text-amber-600 disabled:opacity-40 dark:border-amber-300 dark:text-amber-300">Deshacer</button></div></section>}
    ...
    {cierreLoading ? (
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-void">
          ...
          <p className="mt-2 font-display text-2xl text-emerald-600 dark:text-emerald-300">{formatMoney(cierre.efectivo?.monto || 0)}</p>
  ...
  <CashierSaleSheet open={saleOpen} busy={busy} ticketPrice={ticketPrice} onClose={() => setSaleOpen(false)} onSubmit={submitGeneralSale}/><CashierFeedback feedback={feedback} canUndo={canUndoFeedback} onClose={resetTerminal} onUndo={undoTransaction}/></div>}</main>
}
