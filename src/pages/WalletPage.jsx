...
export default function WalletPage() {
  ...
  const fallback = {
    token,
    codigo: `N001-${String(token).slice(-8).toUpperCase().padStart(4, '0')}`,
  ...
  return (
    <section className="py-10 md:py-16"><div className="container-page max-w-xl">
      ...
      <article className="border border-white/15 bg-floor shadow-[12px_12px_0_rgba(34,211,238,.15)]">
        <div className="border-b border-dashed border-white/20 p-6 text-center"><p className="font-mono text-[10px] uppercase tracking-[.2em] text-muted">Ticket {ticket.codigo}</p><h1 className="display-title mt-4 text-5xl">{event.nombre}</h1><p className="mt-3 text-sm text-muted">{event.fecha_corta || '—'} · {event.horario || '—'} HS · {event.club || event.boliche || '—'}</p></div>
