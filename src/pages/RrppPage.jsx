...
function normalizeRrppEvent(event, index) {
  ...
  const linksArray = Array.isArray(source.links) ? source.links : []
  const listaLink = linksArray.find((l) => l.tipo === 'lista' && l.activo !== false)
  const derivedSlug = slug || listaLink?.slug || null
  ...
  const eventId = firstDefined(source.evento_id, source.id, source.pk, null)
  const internalKey = String(firstDefined(eventId, derivedSlug, `position-${index}`))

  return {
    id: eventId,
    _key: internalKey,
    slug: derivedSlug,
    nombre: firstDefined(source.evento_nombre, source.nombre, source.nombre_evento, source.titulo, source.name, 'Evento sin nombre'),
    fecha: firstDefined(source.evento_fecha, source.fecha, source.fecha_evento, source.fecha_inicio, source.datetime, source.date, null),
    club: firstDefined(source.club, source.venue_name, source.lugar?.nombre, typeof source.lugar === 'string' ? source.lugar : null, null),
    link_personal: suppliedLink || (derivedSlug ? `/lista/${derivedSlug}` : null),
    cupo_max: optionalNumber(source.cupo_max, source.cupo, source.limite, source.capacity),
    anotados: optionalNumber(source.estadisticas?.anotados, source.anotados, source.total_anotados, source.inscriptos, source.registrados),
    ingresados: optionalNumber(source.estadisticas?.ingresados, source.ingresados, source.total_ingresados, source.checked_in),
    pendientes: optionalNumber(source.estadisticas?.pendientes, source.pendientes, source.total_pendientes, source.pending),
    rebotados: optionalNumber(source.estadisticas?.rebotados, source.rebotados, source.total_rebotados, source.rechazados, source.rejected),
    comision: {
      tipo: firstDefined(commissionSource.tipo, commissionSource.type, source.tipo_comision, source.comision_tipo, null),
      valor: optionalNumber(commissionSource.valor, commissionSource.value, source.valor_comision, source.comision_valor),
...
export default function RrppPage() {
    ...
    try {
      ...
      setEvents((current) => current.map((item) => {
        ...
        const returnedEvent = result?.evento && typeof result.evento === 'object'
          ...
          : null
        // REQ-7.5: un invitado recién anotado a mano queda SIEMPRE 'pendiente'
        // (estado consistente hasta que el guardia lo valide), sin importar si el
        // backend devolvió el estado en la respuesta.
        const returnedGuest = result?.invitado && typeof result.invitado === 'object'
          ? { ...normalizeGuest(result.invitado, 0), estado: firstDefined(result.invitado.estado, 'pendiente') }
          : null
        return {
          ...(returnedEvent || item),
  ...
  return <main className="min-h-[100dvh] bg-white dark:bg-void dark:bg-club-grid dark:bg-[size:32px_32px]"><header className="sticky top-0 z-40 border-b border-gray-200 bg-white/95 backdrop-blur dark:border-white/10 dark:bg-void/95"><div className="container-page flex min-h-20 items-center gap-3 py-3"><div className="grid size-11 shrink-0 place-items-center border border-strobe text-strobe"><Icon name="users" size={22}/></div><div className="min-w-0"><p className="font-display text-xl leading-none text-gray-900 dark:text-paper-text sm:text-2xl">PANEL RRPP</p><p className="mt-1 truncate font-mono text-[9px] uppercase tracking-[.16em] text-gray-500 dark:text-muted">{session?.nombre || 'RRPP'} · Gestión de listas</p></div><div className="ml-auto hidden items-center gap-2 sm:flex"><span className={panelStatus === 'live' ? 'status-dot' : 'size-2 bg-gray-400 dark:bg-muted'}/><span className="font-mono text-[9px] uppercase tracking-wider text-gray-500 dark:text-muted">{statusLabel}</span></div><button type="button" onClick={logout} className="ml-2 grid size-11 shrink-0 place-items-center border border-gray-200 text-gray-500 transition hover:border-door-red hover:text-door-red dark:border-white/15 dark:text-muted" aria-label="Cerrar sesión"><Icon name="logout" size={18}/></button></div></header>

  <div className="container-page py-5 sm:py-8"><div className="mb-5 flex items-center gap-2 sm:hidden"><span className={panelStatus === 'live' ? 'status-dot' : 'size-2 bg-gray-400 dark:bg-muted'}/><span className="font-mono text-[9px] uppercase tracking-wider text-gray-500 dark:text-muted">{statusLabel}</span></div>
  ...
  {panelStatus === 'loading' && events.length === 0 ? <section className="panel grid min-h-72 place-items-center p-8 text-center"><div><div className="mx-auto size-10 animate-spin border-2 border-gray-200 border-t-strobe dark:border-white/10"/><p className="mt-5 font-mono text-[10px] uppercase tracking-widest text-gray-500 dark:text-muted">Cargando eventos asignados</p></div></section> : events.length === 0 ? <section className="panel grid min-h-72 place-items-center p-8 text-center"><div><Icon name="calendar" size={38} className="mx-auto text-gray-400 dark:text-muted"/><h1 className="display-title mt-5 text-3xl">SIN EVENTOS ASIGNADOS</h1><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-gray-500 dark:text-muted">Tu cuenta no tiene eventos disponibles en este momento.</p></div></section> : <div className="lg:grid lg:grid-cols-[290px_minmax(0,1fr)] lg:gap-6"><aside className="min-w-0"><div className="mb-3 flex items-center justify-between"><div><p className="eyebrow">Mis eventos</p><h1 className="display-title mt-2 text-2xl">EVENTOS ASIGNADOS</h1></div><span className="font-display text-2xl text-muted">{events.length}</span></div><div className="flex gap-3 overflow-x-auto pb-4 lg:sticky lg:top-24 lg:max-h-[calc(100dvh-120px)] lg:flex-col lg:overflow-y-auto lg:overflow-x-hidden lg:pr-1">{events.map((item) => <EventOption key={item._key} event={item} selected={item._key === selectedEvent?._key} onSelect={() => chooseEvent(item._key)}/>)}</div></aside>

  {selectedEvent && <div className="min-w-0"><section className="panel p-5 sm:p-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="eyebrow">Evento seleccionado</p><h2 className="display-title mt-3 text-4xl sm:text-5xl">{selectedEvent.nombre}</h2><p className="mt-3 font-mono text-[10px] uppercase tracking-wider text-gray-500 dark:text-muted">{displayDate(selectedEvent.fecha)}{selectedEvent.club ? ` · ${selectedEvent.club}` : ''}</p></div><span className={`w-fit border px-3 py-2 font-mono text-[9px] font-bold uppercase ${panelStatus === 'demo' ? 'border-amber-300 text-amber-300' : 'border-emerald-400 text-emerald-300'}`}>{panelStatus === 'demo' ? 'Datos demo' : 'Actualización 15s'}</span></div>

  <div className="mt-7 border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-void"><p className="font-mono text-[9px] font-bold uppercase tracking-[.14em] text-gray-500 dark:text-muted">Tu link personal</p>{selectedEvent.link_personal ? <><p className="mt-2 break-all font-mono text-xs text-strobe">{absoluteListLink(selectedEvent.link_personal)}</p><div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={copyLink} className="btn-primary px-3"><Icon name="share" size={16}/>Copiar link</button><a href={absoluteListLink(selectedEvent.link_personal)} target="_blank" rel="noreferrer" className="btn-secondary px-3"><Icon name="arrow" size={16}/>Abrir enlace</a></div></> : <p className="mt-2 text-sm text-gray-500 dark:text-muted">El backend no informó un link para este evento.</p>}</div>

  <div className="mt-6 flex items-end justify-between gap-4"><div><p className="font-mono text-[9px] uppercase tracking-wider text-gray-500 dark:text-muted">Cupo utilizado</p><p className="mt-1 font-display text-3xl text-gray-900 dark:text-paper-text">{displayCount(selectedEvent.anotados)} <span className="text-lg text-gray-400 dark:text-muted">/ {displayCount(selectedEvent.cupo_max)}</span></p></div><p className="font-display text-3xl text-strobe">{capacityPercent == null ? '—' : `${Math.round(capacityPercent)}%`}</p></div><div className="mt-3 h-2 bg-gray-100 dark:bg-white/5"><div className="h-full bg-gradient-to-r from-uv to-strobe transition-[width] duration-500" style={{ width: `${capacityPercent || 0}%` }}/></div>

  <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-5"><Metric label="Anotados" value={displayCount(selectedEvent.anotados)} accent/><Metric label="Ingresados" value={displayCount(selectedEvent.ingresados)}/><Metric label="Pendientes" value={displayCount(selectedEvent.pendientes)}/><Metric label="Efectividad" value={effectiveness == null ? '—' : `${effectiveness}%`}/><Metric label="Comisión acum." value={selectedEvent.comision.acumulada == null ? '—' : formatMoney(selectedEvent.comision.acumulada)} accent/></div>{(commissionType || selectedEvent.comision.valor != null) && <p className="mt-3 overflow-hidden truncate text-right font-mono text-[9px] uppercase text-gray-500 dark:text-muted">Comisión: {commissionType || 'tipo no informado'}{selectedEvent.comision.valor != null ? ` · ${formatMoney(selectedEvent.comision.valor)}` : ''}</p>}
  </section>
