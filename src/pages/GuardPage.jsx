...
function FeedbackScreen({ feedback, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-void p-6 text-center" role="alert" aria-live="assertive">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-4 font-mono text-sm font-bold uppercase leading-6 text-paper-text">{feedback.message}</p>
        {feedback.detail && <p className="mt-3 text-sm leading-6 text-muted">{feedback.detail}</p>}
...
function RejectSheet({ attendee, busy, onCancel, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-black/80" role="dialog" aria-modal="true" aria-label="Motivo del rechazo">
      <div className="mx-auto w-full max-w-[430px] border-t-2 border-door-red bg-floor p-5 pb-[max(20px,env(safe-area-inset-bottom))]">
        <div className="mb-5 flex items-start justify-between gap-4"><div><p className="eyebrow text-door-red">Rebotar acceso</p><h2 className="display-title mt-2 text-3xl">{attendee.nombre}</h2></div><button type="button" onClick={onCancel} className="grid size-11 shrink-0 place-items-center border border-white/15 text-muted" aria-label="Cancelar rechazo"><Icon name="close" /></button></div>
        <div className="grid grid-cols-2 gap-2">{REJECTION_REASONS.map((item) => <button type="button" key={item} onClick={() => setReason(item)} className={`min-h-16 border px-3 font-mono text-[10px] font-bold uppercase leading-4 ${reason === item ? 'border-door-red bg-door-red/15 text-door-red' : 'border-white/15 text-muted'}`}>{item}</button>)}</div>
    ...
    <main className="min-h-[100dvh] bg-black sm:py-5">
      <div className="relative mx-auto min-h-[100dvh] w-full max-w-[430px] overflow-hidden bg-void sm:min-h-[calc(100dvh-40px)] sm:border sm:border-white/10">
        ...
        <section className="p-4 pb-[max(24px,env(safe-area-inset-bottom))]">
          <div className="grid grid-cols-2 border border-white/15 p-1" aria-label="Método de identificación">
            <button type="button" onClick={() => switchMode('scan')} className={`flex min-h-14 items-center justify-center gap-2 font-mono text-[10px] font-bold uppercase tracking-wider ${mode === 'scan' ? 'bg-uv text-white' : 'text-muted'}`}><Icon name="ticket" size={17}/> Escanear QR</button>
            <button type="button" onClick={() => switchMode('dni')} className={`flex min-h-14 items-center justify-center gap-2 font-mono text-[10px] font-bold uppercase tracking-wider ${mode === 'dni' ? 'bg-uv text-white' : 'text-muted'}`}><Icon name="search" size={17}/> Buscar en lista</button>
          </div>
          ...
          {attendee && <article className="mt-4 border-2 border-strobe bg-floor"><div className="border-b border-dashed border-white/20 p-5"><div className="mb-4 flex items-center justify-between"><span className="font-mono text-[9px] font-bold uppercase tracking-wider text-strobe">Resultado inmediato</span><span className={`border px-2 py-1 font-mono text-[9px] font-bold uppercase ${attendee.estado === 'pendiente' ? 'border-amber-300 text-amber-300' : 'border-door-red text-door-red'}`}>{attendee.estado}</span></div><h2 className="display-title text-4xl">{attendee.nombre}</h2><dl className="mt-5 grid grid-cols-2 gap-4 font-mono"><div><dt className="text-[9px] uppercase text-muted">DNI</dt><dd className="mt-1 text-sm font-bold">{attendee.dni}</dd></div><div><dt className="text-[9px] uppercase text-muted">Ingreso</dt><dd className="mt-1 text-sm font-bold">{attendee.tipo_ingreso}</dd></div><div className="col-span-2"><dt className="text-[9px] uppercase text-muted">RRPP</dt><dd className="mt-1 text-sm font-bold">{attendee.rrpp_nombre || 'Venta directa'}</dd></div></dl></div>{attendee.estado === 'pendiente' ? <div className="grid gap-3 p-4"><button type="button" disabled={busy} onClick={() => executeAction('aprobar')} className="flex min-h-20 items-center justify-center gap-3 bg-uv px-5 font-display text-2xl text-white disabled:opacity-50"><Icon name="check" size={28} strokeWidth={2.5}/> APROBAR</button><button type="button" disabled={busy} onClick={() => setRejecting(true)} className="flex min-h-20 items-center justify-center gap-3 bg-door-red px-5 font-display text-2xl text-white disabled:opacity-50"><Icon name="close" size={28} strokeWidth={2.5}/> REBOTAR</button></div> : <div className="p-4"><p className="border-l-2 border-door-red bg-door-red/10 p-3 text-sm text-door-red">Esta entrada ya tiene estado “{attendee.estado}”. No se puede aprobar nuevamente.</p><button type="button" onClick={resetIdentification} className="btn-secondary mt-3 min-h-14 w-full">Buscar otra persona</button></div>}</article>}
        </section>
      </div>
