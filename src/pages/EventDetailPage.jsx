...
function ListForm({ event, listaSlug, onClose }) {
  ...
  const submit = async (e) => {
    ...
    setMessage('')
    try {
      await api.post(`/lista/${listaSlug}/anotar/`, form)
      setState('success')
    } catch (error) {
      if (error.status === 0) { setState('success'); return }
      if (error.status === 409) {
        setState('duplicate')
      ...
      setState('error')
      setMessage(error.message || 'No pudimos anotarte.')
    }
  }

  if (state === 'duplicate') return (
    <div className="py-6 text-center"><div className="mx-auto mb-5 grid size-16 place-items-center border-2 border-amber-400 text-amber-400"><Icon name="users" size={34} /></div><p className="eyebrow mb-3">Ya figurás en la lista</p><h2 className="display-title text-4xl">YA ESTÁS ANOTADO</h2><p className="mt-4 text-sm leading-6 text-gray-500 dark:text-muted">{message} Presentá tu DNI en la puerta el día del evento.</p><button onClick={onClose} className="btn-primary mt-7 w-full">Listo</button></div>
  )
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div className="mt-7 grid gap-3 sm:grid-cols-2">
        ...
        <input required inputMode="numeric" minLength={7} maxLength={8} className="field sm:col-span-2" placeholder="DNI SIN PUNTOS" value={form.dni} onChange={update('dni')} />
    ...
    <>
      <section className="relative min-h-[540px] overflow-hidden border-b border-white/10">
        ...
        <div className="container-page relative flex min-h-[540px] flex-col justify-between py-8">
          ...
          <div className="max-w-4xl pb-4"><p className="eyebrow mb-4">{event.genero || '—'} · {event.ciudad || '—'}</p><h1 className="display-title text-6xl sm:text-8xl lg:text-[112px]">{event.nombre}</h1><div className="mt-6 flex flex-wrap gap-x-7 gap-y-3 font-mono text-xs uppercase text-paper-text"><span className="flex items-center gap-2"><Icon name="pin" size={16} className="text-door-red" />{event.club || '—'}</span><span className="flex items-center gap-2"><Icon name="calendar" size={16} className="text-strobe" />{event.fecha_corta || '—'}</span><span className="flex items-center gap-2"><Icon name="clock" size={16} className="text-uv" />{event.horario || '—'} HS</span><span>+18</span></div></div>
        </div>
      </section>

      <section className="py-12 md:py-16"><div className="container-page grid gap-10 lg:grid-cols-[1fr_430px]">
        <div>
          <p className="eyebrow mb-3">Sobre la noche</p><p className="max-w-2xl text-lg leading-8 text-muted">{event.descripcion || 'Noche inolvidable en ' + (event.club || 'nuestro venue') + '.'}</p>
          <h2 className="display-title mt-12 text-4xl">LINE-UP</h2><div className="mt-5 flex flex-wrap gap-2">{(event.line_up || event.lineup || []).length ? (event.line_up || event.lineup).map((artist, i) => <span key={artist} className={`border px-4 py-3 font-mono text-xs font-bold uppercase ${i === 0 ? 'border-strobe text-strobe' : 'border-white/20 text-paper-text'}`}>{artist}</span>) : <span className="border border-white/20 px-4 py-3 font-mono text-xs text-muted">A confirmar</span>}</div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} label="Sumarme a la lista"><ListForm event={event} listaSlug={ref || event.lista_slug} onClose={() => setModalOpen(false)} /></Modal>
    </>
  )
}
