...
export default function EventCard({ event, featured = false }) {
  return (
<Link to={`/evento/${event.id}`} className={`group relative isolate block overflow-hidden border border-gray-200 bg-gray-50 dark:border-white/10 dark:bg-floor ${featured ? 'h-[350px] md:h-[430px]' : 'h-[390px]'}`}>
      <img src={event.imagen || undefined} alt="" className="absolute inset-0 -z-20 size-full object-cover grayscale-[35%] transition duration-700 group-hover:scale-105 group-hover:grayscale-0" />
      ...
      <div className="absolute left-0 top-0 border-b border-r border-gray-300 bg-white/80 px-3 py-2 font-mono text-[11px] font-bold text-strobe backdrop-blur dark:border-white/20 dark:bg-void/80">{event.fecha_corta || '—'}</div>
      <div className="absolute inset-x-0 bottom-0 p-5 md:p-6">
        ...
        <p className="mt-3 text-sm text-gray-500 dark:text-muted">{event.genero || '—'} · {(event.line_up || event.lineup || []).join(' / ') || '—'}</p>
