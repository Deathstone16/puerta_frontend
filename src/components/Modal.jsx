...
export default function Modal({ open, onClose, children, label = 'Ventana modal' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4 backdrop-blur-sm dark:bg-void/90" role="dialog" aria-modal="true" aria-label={label} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="relative w-full max-w-lg max-h-[calc(100dvh-32px)] overflow-y-auto border-2 border-strobe bg-white p-6 shadow-[10px_10px_0_#8B5CF6] dark:bg-void md:p-9">
